...
from DeepNet.deeptoxic.models import torch_base
from DeepNet.deeptoxic.models.pytorch import gru
from DeepNet.deeptoxic.models.pytorch import attention
...
class GRUClassifier(torch_base.BaseModel):
    ...
    def __init__(self, input_size, hidden_size, embedding):
        ...
        # use the STL GRU cell
        self.gru = nn.GRU(input_size=input_size, hidden_size=hidden_size,
                          batch_first=True, num_layers=2, dropout=0.35, bidirectional=True)
        self.attn = attention.DotAttention(hidden_size=2*hidden_size, require_mask=False)
    ...
    def forward(self, _input):
        ...
        last = out[:, -1, :]  # since the output is batch first, 'out' is batch_size * sequence_length * hidden_embed×2
        attn, _ = self.attn.forward(out, lengths)
        out_max, _ = torch.max(out, dim=1)
        concatenated = torch.cat([last, out_max, attn], dim=1)
...
class BayesianGRUClassifier(torch_base.BaseModel):
    def __init__(self, input_size, hidden_size, embedding):
        ...
        self.classifier = nn.Sequential(
            OrderedDict([
                ('gru_dropout', nn.Dropout(0.5)),
                ('h1', nn.Linear(self.hidden_size * 8, 72)),
                ('relu1', nn.ReLU()),
                ('out', nn.Linear(72, 6)),
            ]))
    ...
    def forward(self, _input, hidden=None, lengths=None):
        ...
        out_max, _ = torch.max(out2, dim=1)
        out_mean = torch.mean(out2, dim=1)
        concatenated = torch.cat([last, attn, out_max, out_mean], dim=1)

        result = self.classifier(concatenated)
        # Make sure whether the last activation function is necessary or not
        return result
