...
from DeepNet.deeptoxic.models.pytorch.dropout import SequentialDropout
...
class BiBayesianGRU(AbstractGRU):
    ...
    def forward(self, x, hx=None, max_length=None, lengths=None):
        batch_size = x.size(0)  # x (after embedding):  batch_size * sequence_length * hidden_size
        seq_length = x.size(1)
        if max_length is None:
            max_length = seq_length
        lefts = []
        ...
        '''
        # left part
        lhx = self.init_hidden(batch_size)  # hidden： 1 * batch_size * hidden_size
        for time in range(max_length):
            # x == 0 when time > length
            new_hx = self.gru_cell(x[:, time, :], hx=lhx)
            # mask to decide update lhx or not, when time > length, lhx is always the same
        ...
        output = torch.cat((lefts, rights), dim=2)  # output: batch_size * sequence_length * hidden_size×2
        return output, lhx
