...
class Attention(Layer):
    ...
    def build(self, input_shape):
        ...
        self.W = self.add_weight((input_shape[-1],),  # 这里写成(shape[-1], )，因为输入必须是个tuple
                                 initializer=self.init,
                                 name='{}_W'.format(self.name),
                                 regularizer=self.W_regularizer,  # 上一个函数中声明的变量的用处
    ...
    def call(self, x, mask=None):
        ...
        # 从reshape之shape为(feature_dim, 1)看出来W是一维的
        eij = K.reshape(K.dot(K.reshape(x, (-1, features_dim)), K.reshape(self.W, (features_dim, 1))), (-1, step_dim))
        # W的维度与词向量维度相吻合，x和W相乘的结果维度与句子中单词个数相吻合。
        # 为什么最后一个必须是-1？若是1则运行报错，采用32的batchsize，80的step_dim,报错说有2560（32*80）个值，是因为算上了batch？
        if self.bias:
            eij += self.b
        ...
        # 这里将a中元素都除以a所有元素之和，使加起来为1
        a /= K.cast(K.sum(a, axis=1, keepdims=True) + K.epsilon(), K.floatx())  # a一维，长度是句子长度；

        a = K.expand_dims(a)  # expand_dims(a, axis), axis默认值为-1，表示增加的维度在最后
        weighted_input = x * a  # tensorflow、numpy中.dot表示矩阵乘法，* 表示按位置相乘，并自动广播broadcast

        return K.sum(weighted_input, axis=1)  # 将特征做平均，输出256 * 1维向量，表示256特征维数
...
tokenizer.fit_on_texts(comments + test_comments)  # fit_on_texts的时候，将train和test的预料都放入
